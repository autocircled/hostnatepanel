<div class="card">
	<div class="card-body">
		...
		<table class="table table-sm table-bordered table-hover">
			...
			<tr>
				<td>
					<span class="d-inline-block text-truncate" style="max-width: 120px;" data-toggle="tooltip" data-placement="top" title="{$service.emailAddress}">{$service.emailAddress}</span>
				</td>
				<td>
				...
				<td>
					<form method="post" action="emailmanagement.php?action=modifyaccount">
						...
						<input type="submit" value="{$LANG.rcmail_modifyemailbutton}" class="btn btn-outline-primary btn-sm"/>
					</form>
				</td>
				<td>
				...
				<td>
					<form method="post" action="emailmanagement.php?action=manageemails" {if $service.status eq "ACTIVE"}onclick="return confirmSuspend();{/if}">
						...
						<input type="hidden" name="itemlimit" value="{$itemlimit}"/>
						<input type="hidden" name="q" value="{$q}"/>
						<input type="submit" value="{if $service.status eq "ACTIVE"}{$LANG.rcmail_suspendbutton}{else}{$LANG.rcmail_unsuspendbutton}{/if}" class="btn btn-outline-warning btn-sm"/>
					</form>
				</td>
				...
				<td colspan="8">
					...
					<div style="float:right;">
						<form id="multiaction" method="post" action="emailmanagement.php?action=manageemails">
							...
							<input type="submit" name="msuspend" value="{$LANG.rcmail_multisuspendbutton}" class="btn btn-warning btn-sm" onclick="return confirmSuspend();" />
						</form>
					</div>
					{/if}
					{if $q neq ""}
					<div style="float:left;">
						<form method="post" action="emailmanagement.php?action=manageemails">
							...
							<input type="hidden" name="domain" value="{$domain}"/>
							<input name="freemailhosting" value="{$freemailhosting}" type="hidden"/>
							<input type="submit" value="{$LANG.rcmail_deletesearchquerybutton}" class="btn btn-secondary btn-sm"/>
						</form>
					</div>
					{/if}				
				</td>
		...
		<div class="col-sm-5">
			...
			<div class="btn-group" role="group" aria-label="pagination">
				<button type="button" class="btn btn-secondary" {if !$prevpage} disabled{/if}><a style="text-decoration:none;color:inherit;" href="{if $prevpage}emailmanagement.php?action=manageemails{if $q}&q={$q}{/if}&amp;page={$prevpage}&domainid={$domainid}&domain={$domain}&freemailhosting={$freemailhosting}&itemlimit={$itemlimit}{else}javascript:return false;{/if}">&larr; {$LANG.previouspage}</a></button>
				<button type="button" class="btn btn-secondary" {if !$nextpage || $nextpage==$pageend} disabled{/if}><a style="text-decoration:none;color:inherit;" href="{if $nextpage != $pageend}emailmanagement.php?action=manageemails{if $q}&q={$q}{/if}&amp;page={$nextpage}&domainid={$domainid}&domain={$domain}&freemailhosting={$freemailhosting}&itemlimit={$itemlimit}{else}javascript:return false;{/if}">{$LANG.nextpage} &rarr;</a></button>
			</div>
		</div>
	</div>
</div>