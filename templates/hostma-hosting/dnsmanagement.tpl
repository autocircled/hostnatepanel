<div class="card">
	<div class="card-body">
		...
		<div class="input-group mb-3">
			<form method="post" action="dnsmanagement.php?action=managednszone">
				...
				<div class="input-group-append">
					<input type="text" name="q" value="{if $q}{$q}{else}{$LANG.rcdns_searchenterdomain}{/if}" class="form-control appended-form-control" onfocus="if(this.value=='{$LANG.rcdns_searchenterdomain}')this.value=''" />
		...
		<table class="table table-sm table-bordered table-hover">
			...
			<tr>
				<td>
					<span class="d-inline-block text-truncate" style="max-width: 120px;" data-toggle="tooltip" data-placement="top" title="{$service.host}.{$domain}">{$service.host}.{$domain}</span>
				</td>
				<td>{if $recordtype eq "TXT"}<div style="width: 250px; word-wrap: break-word;">{$service.value}</div>{else}{$service.value}{/if}</td>
				<td>{$service.timetolive}</td>
				<td>{if $service.status eq "Active"}<span class="label label-success">{$LANG.clientareaactive}</span>{else}<span class="label label-default">{$LANG.clientareasuspended}</span>{/if}</td>
				<td>
					<form method="POST" action="dnsmanagement.php?action=managednszonemodify">
						...
						<input type="submit" value="{$LANG.rcdns_modifybutton}" class="btn btn-outline-primary btn-sm"/>
					</form>
				</td>
				<td>
					<form method="POST" action="dnsmanagement.php?action=managednszone" onclick="return confirmDelete();">
						...
						<input type="hidden" name="host" value="{$service.host}"/>
						<input type="hidden" name="value" value="{$service.value}"/>
						<input type="submit" value="{$LANG.rcdns_deletebutton}" class="btn btn-outline-danger btn-sm"/>
					</form>
				</td>
				...
				<td colspan="7">
					...
					<div style="float:left;">
						<form method="post" action="dnsmanagement.php?action=managednszone">
							...
							<input type="hidden" name="domain" value="{$domain}"/>
							<input name="freednshosting" value="{$freednshosting}" type="hidden"/>
							<input type="submit" value="{$LANG.rcdns_deletesearchquerybutton}" class="btn btn-success btn-sm"/>
						</form>
					</div>
					{/if}
				</td>
		...
		<div class="col-sm-5">
			...
			<div class="btn-group" role="group" aria-label="pagination">
				<ul class="pagination" style="margin-top:0;">
					<button type="button" class="btn btn-secondary" {if !$prevpage} disabled{/if}><a style="text-decoration:none;color:inherit;" href="{if $prevpage}dnsmanagement.php?action=managednszone{if $q}&q={$q}{/if}&amp;page={$prevpage}&domainid={$domainid}&domain={$domain}&freednshosting={$freednshosting}&nsrecordtype={$recordtype}&itemlimit={$itemlimit}{else}javascript:return false;{/if}">&larr; {$LANG.previouspage}</a></button>
					<button type="button" class="btn btn-secondary" {if !$nextpage || $nextpage==$pageend} disabled{/if}><a style="text-decoration:none;color:inherit;" href="{if $nextpage != $pageend}dnsmanagement.php?action=managednszone{if $q}&q={$q}{/if}&amp;page={$nextpage}&domainid={$domainid}&domain={$domain}&freednshosting={$freednshosting}&nsrecordtype={$recordtype}&itemlimit={$itemlimit}{else}javascript:return false;{/if}">{$LANG.nextpage} &rarr;</a></button>
				</ul>
			</div>
		</div>
	</div>
